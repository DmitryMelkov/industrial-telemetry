...
Seed-датчики (T-101 и др.) берут профиль сигнала из `SEED_SENSORS`. **T-101** раз в несколько минут держит значение вне warning (иногда critical) дольше `ALERT_OPEN_DEBOUNCE_SECONDS` (~20 с), затем возвращается в норму — так журнал алертов наполняется без отключения debounce. Остальные seed-датчики обычно остаются внутри порогов. Датчик, созданный в Admin с `isActive=true`, попадает в батч после refresh каталога (`GENERATOR_SENSOR_REFRESH_MS`, по умолчанию 10 с) — править `SEED_SENSORS` не нужно. Для новых датчиков `baseValue`/`noise` берутся из дефолтов по `metric` (без демо-экскурсий).
...
Текущая реализация демо-стенда намеренно простая:
